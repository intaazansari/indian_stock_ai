...
import type { Company, CompanySearchResult, PeerCompany, PricePoint, LivePrice } from "@/types/company";
...
export const companiesApi = {
  ...
  getLivePrice: async (symbol: string): Promise<LivePrice> => {
    const { data } = await apiClient.get<LivePrice>(`/companies/${symbol}/live-price`);
    return data;
  },
